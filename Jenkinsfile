            ...
            steps {
                script {
                    if (BRANCH_NAME == "dev") {
                        ...
                        error "El nombre de la rama no es el correcto. Solo pueden ser 'main' o 'dev'"
                    }
                    env.project_id = sh(script: 'jq -r ".project_id" $GOOGLE_APPLICATION_CREDENTIALS', returnStdout: true).trim()
                    env.dockerimg_name = "${artifact_registry}/${env.project_id}/${repo}/${service_name}:${GIT_COMMIT}"
                }
